...
public class ForceModifierZoneContents implements IInventory {
     ...
     */
    public static ForceModifierZoneContents createForTileEntity(int size,
                                                                Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
                                                                Notify markDirtyNotificationLambda) {
        return new ForceModifierZoneContents(size, canPlayerAccessInventoryLambda, markDirtyNotificationLambda);
    }
     ...
     * @return the new ChestContents
     */
    public static ForceModifierZoneContents createForClientSideContainer(int size) {
        return new ForceModifierZoneContents(size);
    }
    ...
    public boolean canPlaceItem(int index, ItemStack stack) {
        return forceControllerComponentContents.isItemValid(index, stack);
    }
    ...
    public int getContainerSize() {
        return forceControllerComponentContents.getSlots();
    }

    @Override
    public boolean isEmpty() {
        for (int i = 0; i < forceControllerComponentContents.getSlots(); ++i) {
            if (!forceControllerComponentContents.getStackInSlot(i).isEmpty()) return false;
        }
        return true;
    }
    ...
    public ItemStack getItem(int index) {
        return forceControllerComponentContents.getStackInSlot(index);
    }

    @Override
    public ItemStack removeItem(int index, int count) {
        if (count < 0) throw new IllegalArgumentException("count should be >= 0:" + count);
        return forceControllerComponentContents.extractItem(index, count, false);
    }

    @Override
    public ItemStack removeItemNoUpdate(int index) {
        int maxPossibleItemStackSize = forceControllerComponentContents.getSlotLimit(index);
        return forceControllerComponentContents.extractItem(index, maxPossibleItemStackSize, false);
    }

    @Override
    public void setItem(int index, ItemStack stack) {
        forceControllerComponentContents.setStackInSlot(index, stack);
    }

    @Override
    public void clearContent() {
        for (int i = 0; i < forceControllerComponentContents.getSlots(); ++i) {
            forceControllerComponentContents.setStackInSlot(i, ItemStack.EMPTY);
        }
    }
    ...
    public ItemStack increaseStackSize(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = forceControllerComponentContents.insertItem(index, itemStackToInsert, false);
    ...
    public boolean doesItemStackFit(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = forceControllerComponentContents.insertItem(index, itemStackToInsert, true);
        return leftoverItemStack.isEmpty();
    }

    // ---------

    private ForceModifierZoneContents(int size) {
        this.forceControllerComponentContents = new ItemStackHandler(size);
    }

    private ForceModifierZoneContents(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda, Notify markDirtyNotificationLambda) {
        this.forceControllerComponentContents = new ItemStackHandler(size);
        this.canPlayerAccessInventoryLambda = canPlayerAccessInventoryLambda;
        this.markDirtyNotificationLambda = markDirtyNotificationLambda;
    }
    ...
    private final ItemStackHandler forceControllerComponentContents;
}